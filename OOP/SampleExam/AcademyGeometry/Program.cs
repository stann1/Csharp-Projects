...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IAreaMeasurable, IFlat
    {
        ...
        {
            get { return this.vertices[0]; }
            private set { this.vertices[0] = value; }
        }
        ...
        public GeometryAPI.Vector3D GetNormal()
        {
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        {
            get { return this.vertices[1]; }
            private set { this.vertices[1] = value; }
        }
        ...
        {
            this.Radius = radius;
            this.height = (this.CenterBottom - this.CenterTop).Magnitude;
        }
    ...
    public class ExtendedFigureController : FigureController
    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        var figureAsIAreaMeasurable = this.currentFigure as IAreaMeasurable;
                        if (figureAsIAreaMeasurable != null)
                        {
                            Console.WriteLine("{0:0.00}", figureAsIAreaMeasurable.GetArea());
                        }
                        else
                        {
                ...
                case "volume":
                    {
                        var figureAsIvolumeMeasurable = this.currentFigure as IVolumeMeasurable;
                        if (figureAsIvolumeMeasurable != null)
                        {
                            Console.WriteLine("{0:0.00}", figureAsIvolumeMeasurable.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "normal":
                    {
                        var figureAsIFlat = this.currentFigure as IFlat;
                        if (figureAsIFlat != null)
                        {
                            Console.WriteLine(figureAsIFlat.GetNormal().ToString());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
